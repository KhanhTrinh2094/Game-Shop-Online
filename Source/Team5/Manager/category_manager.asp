...
If Request.QueryString("active") <> "" Then
	...
	If (Not MM_abortEdit) Then
		...
		MM_editCmd.CommandText = "Update dbo.Category SET status = 1 WHERE CategoryID = ?"
...
If Request.QueryString("deactive") <> "" Then
	...
	If (Not MM_abortEdit) Then
		...
		Set MM_editCmd = Server.CreateObject ("ADODB.Command")
		MM_editCmd.ActiveConnection = MM_CONNECT_STRING
		MM_editCmd.CommandText = "Update dbo.Category SET status = 0 WHERE CategoryID = ?"
		MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 5, 1, -1, ID) ' adDouble
		MM_editCmd.Execute
		MM_editCmd.ActiveConnection.Close
...
Set rsLis_cmd = Server.CreateObject ("ADODB.Command")
rsLis_cmd.ActiveConnection = MM_CONNECT_STRING
rsLis_cmd.CommandText = "SELECT * FROM dbo.Category ORDER BY CategoryID DESC" 
rsLis_cmd.Prepared = true

Set rsLis = rsLis_cmd.Execute
rsLis_numRows = 0
%>
<%
...
Repeat1__numRows = -1
Repeat1__index = 0
rsLis_numRows = rsLis_numRows + Repeat1__numRows
%>
                                        ...
                                        <div class="panel panel-default" id="panel-reorder">
                                            <div class="panel-heading">
                                                ...
                                                <div class="panel-ctrls">
                                                	<i class="separator"></i>
                                                    <a href="category_add.asp"><button class="btn-default btn">Add New</button></a>
                                                </div>
                                            </div>
                                            <div class="panel-body panel-no-padding">
                                                <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered datatables" id="colreorder">
                                                    ...
                                                    <tbody>
                                                    <% While ((Repeat1__numRows <> 0) AND (NOT rsLis.EOF)) %>
                                                        <tr>
                                                          <td><%=(rsLis.Fields.Item("CategoryID").Value)%></td>
                                                          <td style="max-width:100px; word-wrap:break-word"><%=(rsLis.Fields.Item("CategoryName").Value)%></td>
                                                          <td style="max-width:300px; word-wrap:break-word"><%=(rsLis.Fields.Item("CategoryDescription").Value)%></td>
                                                          <td align="center">
                                                          	<a href="category_edit.asp?id=<%=(rsLis.Fields.Item("CategoryID").Value)%>"><button class="btn-default btn">Edit</button></a> <a href="category_delete.asp?id=<%=(rsLis.Fields.Item("CategoryID").Value)%>">
                                                            <%
														  	If rsLis.Fields.Item("Status") = 0 Then
														  %>
                                                          		<a href="javascript:active(<%=(rsLis.Fields.Item("CategoryID").Value)%>)"><button class="btn-default btn">Active</button></a>
														  <%
															Else
														  %>
                                                          		<a href="javascript:deactive(<%=(rsLis.Fields.Item("CategoryID").Value)%>)"><button class="btn-default btn">Deactive</button></a>
														  <%
															End IF
														  %>
                                                          </td>
                        ...
                        <script type="text/javascript">
							function active(id){
								if(confirm("Are you sure you want to active this category ?")){
									window.location = "category_manager.asp?active=" + id;
								}
							}
							function deactive(id){
								if(confirm("Are you sure you want to deactive this category ?")){
									window.location = "category_manager.asp?deactive=" + id;
								}
							}
						</script>
						<!-- InstanceEndEditable -->
