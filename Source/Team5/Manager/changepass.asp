...
If (Session("AD_Username") <> "") Then 
  rsPass__MMColParam = Session("AD_Username")
End If
%>
<%
Dim rsPass
...
Set rsPass_cmd = Server.CreateObject ("ADODB.Command")
rsPass_cmd.ActiveConnection = MM_CONNECT_STRING
rsPass_cmd.CommandText = "SELECT * FROM dbo.Admin WHERE Username = ?" 
rsPass_cmd.Prepared = true
rsPass_cmd.Parameters.Append rsPass_cmd.CreateParameter("param1", 200, 1, 30, rsPass__MMColParam) ' adVarChar

Set rsPass = rsPass_cmd.Execute
rsPass_numRows = 0
%>
...
If (CStr(Request("MM_update")) = "form1") Then
  If(CStr((Request.Form("txtCurrentPass")) = (rsPass.Fields.Item("Password").Value))) then
	  If (Not MM_abortEdit) Then
		...
		MM_editCmd.CommandText = "UPDATE dbo.[Admin] SET Password = ? WHERE Username = ?" 
		MM_editCmd.Prepared = true
		MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 201, 1, 30, Request.Form("txtNewPass")) ' adLongVarChar
		MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param2", 5, 1, -1, Cstr(Session("AD_Username"))) ' adDouble
		MM_editCmd.Execute
		MM_editCmd.ActiveConnection.Close
	
		' append the query string to the redirect URL
		Dim MM_editRedirectUrl
		MM_editRedirectUrl = "Manager/index.asp"
		If (Request.QueryString <> "") Then
...
<html lang="en"><!-- InstanceBegin template="/Templates/Admins.dwt.asp" codeOutsideHTMLIsLocked="false" -->
    <head>
        ...
        <!-- Initialize scripts for this page-->
        	<!-- InstanceBeginEditable name="PageStyle" -->Style In Page<!-- InstanceEndEditable -->
    ...
    <body class="infobar-offcanvas">
                                ...
                                <div id="panel-advancedoptions">
                                    <div class="panel panel-default" data-widget-editbutton="false" id="p1">
                                        ...
                                        <div class="panel-body">
                                          <form name="form1" METHOD="POST" action="<%=MM_editAction%>" class="form-horizontal row-border">
                                                <div class="form-group">
                                                    <label class="col-sm-2 control-label">Mật khẩu cũ</label>
                                                    <div class="col-sm-8">
                                                        <input type="text" name="txtCurrentPass" class="form-control" />
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                    ...
                                                    <div class="col-sm-8">
                                                        <input type="password" name="txtConfirmPass" class="form-control" />
                                                    </div>
                                              </div>
                                            <input type="hidden" name="MM_update" value="form1">
                                            <input type="hidden" name="MM_recordId" value="<%= rsPass.Fields.Item("AdminID").Value %>">
                                          </form>
                                            <div class="panel-footer">
                                                <div class="row">
                                                    <div class="col-sm-8 col-sm-offset-2">
                                                        <input type="submit" class="btn-primary btn" value="Xác Nhận" />
                                                        <button class="btn-default btn">Huỷ bỏ</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
        ...
        <!-- Initialize scripts for this page-->
        	<!-- InstanceBeginEditable name="PageScript" -->Script In Page<!-- InstanceEndEditable -->
        <!-- End loading page level scripts-->
    </body>
<!-- InstanceEnd --></html>
<%
