...
If (CStr(Request("MM_update")) = "form1") Then
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "UPDATE dbo.Game SET GameTitle = ?, GameImage = ?, CategoryID = ?, GamePrice = ?, GameDetail = ? WHERE GameID = ?" 
    MM_editCmd.Prepared = true
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 202, 1, 200, Request.Form("title")) ' adVarWChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param2", 201, 1, 200, Request.Form("txtLink")) ' adLongVarChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param3", 5, 1, -1, MM_IIF(Request.Form("category"), Request.Form("category"), null)) ' adDouble
    ...
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param5", 203, 1, 1073741823, Request.Form("detail")) ' adLongVarWChar
    ...
    MM_editRedirectUrl = "game_manager.asp"
    If (Request.QueryString <> "") Then
...
rsGame_cmd.CommandText = "SELECT * FROM dbo.Game WHERE GameID = ?" 
...
rsCategory_cmd.CommandText = "SELECT * FROM dbo.Category WHERE Status=1" 
                ...
                <div class="static-content-wrapper">
                    <div class="static-content">
						...
						function kt()
						{
							...
							if(title.length > 200)
							{
								alert("Tiêu đề game quá dài");
								document.form1.title.focus();
								return false;
							}
							if(detail.length == 0)
							{
								alert("Vui lòng Nhập nội dung game");
								document.form1.detail.focus();
								return false;
							}
							if(txtLink.length == 0)
							{
								alert("Vui lòng chọn hình đại diện cho game này");
							...
							if(price.length == 0){
								alert("Vui lòng nhập giá tiền cho game");
							...
							if(parseInt(price) < 0 || parseInt(price) > 1000000000){
								alert("Giá tiền phải > 0 và < 1.000.000.000");
								document.form1.price.focus();
								return false;
							}
							return true;
                                                ...
                                                <div class="tab-content">
                                                    <div class="tab-pane active" id="horizontal-form">
                                                        	<div class="form-group">
                                                                <label class="col-sm-2 control-label" style="    padding-top: 0px;" >Title : </label>
                                                                <div class="col-sm-1">
                                                                    
                                                                </div>
                                                                <div class="col-sm-9">
                                                                    <input name="title" type="text" class="form-control" placeholder="Game Title" value="<%=(rsGame.Fields.Item("GameTitle").Value)%>" />
                                                                </div>
                                                            </div>
                                                            <div class="form-group">
                                                                <label class="col-sm-2 control-label" style="    padding-top: 0px;" >Image : </label>
                                                                <div class="col-sm-1">
                                                                    
                                                                </div>
                                                                <div class="col-sm-9">
                                                                	<img src="../upload/<%=(rsGame.Fields.Item("GameImage").Value)%>" style="width:50px; height:50px;" />
                                                                    <input type="file" name="txtImage" id="txtImage" onchange="cut_string();">
                                  									<input type="hidden" name="txtLink" id="txtLink" value="<%=(rsGame.Fields.Item("GameImage").Value)%>" />
                                                                </div>
                                                            </div>
                                                            <div class="form-group">
                                                                <label class="col-sm-2 control-label" style="    padding-top: 0px;" >Category : </label>
                                                                <div class="col-sm-1">
...
%>
                                                                	  <option value="<%=(rsCategory.Fields.Item("CategoryID").Value)%>" <%If (Not isNull((rsGame.Fields.Item("CategoryID").Value))) Then If (CStr(rsCategory.Fields.Item("CategoryID").Value) = CStr((rsGame.Fields.Item("CategoryID").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%> ><%=(rsCategory.Fields.Item("CategoryName").Value)%></option>
                                                                	  <%
  rsCategory.MoveNext()
Wend
...
%>
                                                            ...
                                                            <div class="form-group">
                                                                <label class="col-sm-2 control-label" style="    padding-top: 0px;" >Price : </label>
                                                                <div class="col-sm-1">
                                                                    
                                                                </div>
                                                                <div class="col-sm-9">
                                                                    <input name="price" type="text" class="form-control" placeholder="Game price" value="<%=(rsGame.Fields.Item("GamePrice").Value)%>" />
                                                                </div>
                                                            </div>
                                                            <div class="form-group">
                                                                <label class="col-sm-2 control-label" style="    padding-top: 0px;" >Detail : </label>
                                                                <div class="col-sm-1">
                                                                    
                                                                </div>
                                                                <div class="col-sm-9">
                                                                    <textarea class="form-control" name="detail" placeholder="Game Detail"><%=(rsGame.Fields.Item("GameDetail").Value)%></textarea>
                                                                </div>
                                                            </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="panel-footer">
                                                    <div class="row">
                                                        <div class="col-sm-8 col-sm-offset-2">
                                                            ...
                                                            <a href="game_manager.asp"><button type="button" class="btn-default btn">Back</button></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <input type="hidden" name="MM_update" value="form1">
                                            <input type="hidden" name="MM_recordId" value="<%= rsGame.Fields.Item("GameID").Value %>">
                                            </form>
                                        </div>
                                    </div>
                                </div>
