...
If Request.QueryString("active") <> "" Then
	...
	If (Not MM_abortEdit) Then
		...
		MM_editCmd.CommandText = "Update dbo.Game SET status = 1 WHERE GameID = ?"
...
If Request.QueryString("deactive") <> "" Then
	...
	If (Not MM_abortEdit) Then
		...
		Set MM_editCmd = Server.CreateObject ("ADODB.Command")
		MM_editCmd.ActiveConnection = MM_CONNECT_STRING
		MM_editCmd.CommandText = "Update dbo.Game SET status = 0 WHERE GameID = ?"
		MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 5, 1, -1, ID) ' adDouble
		MM_editCmd.Execute
		MM_editCmd.ActiveConnection.Close
...
rsGame_cmd.CommandText = "SELECT a.*, b.CategoryName FROM dbo.Game a JOIN dbo.Category b ON a.CategoryID = b.CategoryID ORDER BY GameID DESC" 
                ...
                <div class="static-content-wrapper">
                    <div class="static-content">
                                        ...
                                        <div class="panel panel-default" id="panel-reorder">
                                            <div class="panel-heading">
                                                ...
                                                <div class="panel-ctrls">
                                                	<i class="separator"></i>
                                                	<a href="game_add.asp"><button class="btn-default btn">Add New</button></a>
                                                </div>
                                            </div>
                                            ...
                                            <%SetLocale("vi-vn") %>
                                                <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered datatables" id="colreorder">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                          <td><%=(rsGame.Fields.Item("GameID").Value)%></td>
                                                          <td style="max-width:200px; word-wrap:break-word"><%=(rsGame.Fields.Item("GameTitle").Value)%></td>
                                                          <td><img src="../upload/<%=(rsGame.Fields.Item("GameImage").Value)%>" style="height:50px; width:50px;" /></td>
                                                          <td><%=FormatCurrency((rsGame.Fields.Item("GamePrice").Value),0)%></td>
                                                          <td><%=(rsGame.Fields.Item("CategoryName").Value)%></td>
                                                          <td align="center" style="width:210px;">
                                                          	<a data-toggle="modal" href="#detail-<%=(rsGame.Fields.Item("GameID").Value)%>"><button class="btn-default btn" type="button">Detail</button></a>
                                                          	<a href="game_edit.asp?id=<%=(rsGame.Fields.Item("GameID").Value)%>"><button class="btn-default btn" type="button">Edit</button></a> 
                                                            <%
														  	If rsGame.Fields.Item("Status") = 0 Then
														  %>
                                                          		<a href="javascript:active(<%=(rsGame.Fields.Item("GameID").Value)%>)"><button class="btn-default btn">Active</button></a>
														  <%
															Else
														  %>
                                                          		<a href="javascript:deactive(<%=(rsGame.Fields.Item("GameID").Value)%>)"><button class="btn-default btn">Deactive</button></a>
														  <%
															End IF
														  %>
                                                            </td>
						...
						<%
							...
							While ((Repeat1__numRows <> 0) AND (NOT rsGame.EOF))
						%>
                                <div class="modal fade" id="detail-<%=(rsGame.Fields.Item("GameID").Value)%>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                                <h2 class="modal-title">Game Detail</h2>
                                            </div>
                                            <div class="modal-body">
                                            	<form class="form-horizontal">
                                                  <div class="form-group">
                                                    ...
                                                    <div class="col-sm-9"><%=(rsGame.Fields.Item("GameID").Value)%></div>
                                                  </div>
                                                  <div class="form-group">
                                                    <div class="col-sm-2">Title : </div>
                                                    <div class="col-sm-1"></div>
                                                    <div class="col-sm-9" style="word-wrap:break-word"><%=(rsGame.Fields.Item("GameTitle").Value)%></div>
                                                  </div>
                                                  <div class="form-group">
                                                    <div class="col-sm-2">Image : </div>
                                                    <div class="col-sm-1"></div>
                                                    <div class="col-sm-9" style="max-width:100%; word-wrap:break-word;"><img src="../upload/<%=(rsGame.Fields.Item("GameImage").Value)%>" style="width:50px; height:50px;" /></div>
                                                  </div>
                                                  <div class="form-group">
                                                    <div class="col-sm-2">Category : </div>
                                                    <div class="col-sm-1"></div>
                                                    <div class="col-sm-9" style="max-width:100%; word-wrap:break-word;"><%=(rsGame.Fields.Item("CategoryName").Value)%></div>
                                                  </div>
                                                  <div class="form-group">
                                                    <div class="col-sm-2">Price : </div>
                                                    <div class="col-sm-1"></div>
                                                    <div class="col-sm-9" style="max-width:100%; word-wrap:break-word;"><%=FormatCurrency((rsGame.Fields.Item("GamePrice").Value),0)%></div>
                                                  </div>
                                                  <div class="form-group">
                                                    <div class="col-sm-2">Detail : </div>
                                                    <div class="col-sm-1"></div>
                                                    <div class="col-sm-9" style="max-width:100%; word-wrap:break-word;"><%=(rsGame.Fields.Item("GameDetail").Value)%></div>
                                                  </div>
                                                </form>
                                            </div>
							...
							function active(id){
								if(confirm("Are you sure you want to active this game ?")){
									window.location = "game_manager.asp?active=" + id;
								}
							}
							function deactive(id){
								if(confirm("Are you sure you want to deactive this game ?")){
									window.location = "game_manager.asp?deactive=" + id;
								}
							}
						</script>
						<!-- InstanceEndEditable -->
