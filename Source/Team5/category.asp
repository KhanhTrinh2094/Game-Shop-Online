...
rsGame_cmd.CommandText = "SELECT * FROM dbo.Game WHERE CategoryID = ? AND Status = 1 ORDER BY GameID DESC" 
...
%>
<%
Dim rsCate__MMColParam
rsCate__MMColParam = "1"
If (Request.QueryString("id") <> "") Then 
  rsCate__MMColParam = Request.QueryString("id")
End If
%>
<%
Dim rsCate
...
Set rsCate_cmd = Server.CreateObject ("ADODB.Command")
rsCate_cmd.ActiveConnection = MM_CONNECT_STRING
rsCate_cmd.CommandText = "SELECT * FROM dbo.Category WHERE CategoryID = ? AND Status = 1" 
rsCate_cmd.Prepared = true
rsCate_cmd.Parameters.Append rsCate_cmd.CreateParameter("param1", 5, 1, -1, rsCate__MMColParam) ' adDouble

Set rsCate = rsCate_cmd.Execute
rsCate_numRows = 0
                                ...
                                <div class="cell">
                                  <h1>Game nhập vai</h1>
                                    <ul class='breadcrumbs'>
                                        ...
                                        <li>
                                            <span>
                                                <a href="" title=""><%=(rsCate.Fields.Item("CategoryName").Value)%></a>
                                            </span>
                                        </li>
                                    </ul>
                                </div>
