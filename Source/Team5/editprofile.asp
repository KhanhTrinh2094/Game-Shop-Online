...
If (CStr(Request("MM_update")) = "form1") Then
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "UPDATE dbo.Member SET MemberName = ?, MemberAddress = ?, MemberMobile = ?, MemberEmail = ? WHERE MemberID = ?" 
    MM_editCmd.Prepared = true
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 202, 1, 50, Request.Form("name")) ' adVarWChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param2", 202, 1, 300, Request.Form("address")) ' adVarWChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param3", 201, 1, 11, Request.Form("mobile")) ' adLongVarChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param4", 201, 1, 60, Request.Form("email")) ' adLongVarChar
                                ...
                                <div class="row">
                                  <div class="large-5 columns large-centered">
                                    <div id="customer-login" class="account-session-module">
                                      <div class="inner">
                                      	<p class="note">Vui lòng nhập vào các nội dung bên dưới.</p>
                                        <form ACTION="<%=MM_editAction%>" name="form1" method='POST' onSubmit="return kt();">
                                            <fieldset>
                                            	<div class="row">
                                                  <div class="large-12 columns field">
                                                    <input type="text" name="username" placeholder="Username" value="<%=(rsMember.Fields.Item("Username").Value)%>" disabled="disabled" />
                                                  </div>
                                                </div>
                                                <div class="row">
                                                  <div class="large-12 columns field">
                                                    <input type="text" name="name" placeholder="Họ têṇ" value="<%=(rsMember.Fields.Item("MemberName").Value)%>" />
                                                  </div>
                                                </div>
                                                <div class="row">
                                                  <div class="large-12 columns field">
                                                    <input type="text" name="address" placeholder="Địa chỉ" value="<%=(rsMember.Fields.Item("MemberAddress").Value)%>" />
                                                  </div>
                                                </div>
                                                <div class="row">
                                                  <div class="large-12 columns field">
                                                    <input type="email" name="mobile" placeholder ="Số điện thoại" value="<%=(rsMember.Fields.Item("MemberMobile").Value)%>"/>
                                                  </div>
                                                </div>
                                                <div class="row">
                                                  <div class="large-12 columns field">
                                                    <input type="email" name="email" placeholder ="Địa chỉ Email" value="<%=(rsMember.Fields.Item("MemberEmail").Value)%>"/>
                                                  </div>
                                                </div>
                                              <div class="form-actions">
                                                  <input type="submit" class="button" value="Sửa thông tin" />
                                                  <div class="secondary-actions">
                                                    <a href="profile.asp"><button type="button" class="button">Trở về</button></a>
                                                  </div>
                                                </div>
                                            </fieldset>
