...
If (CStr(Request("MM_insert")) = "form1") Then
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "INSERT INTO dbo.Feedback (FeedbackTitle, FeedbackDetail, MemberID) VALUES (?, ?, ?)" 
    MM_editCmd.Prepared = true
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 202, 1, 100, Request.Form("title")) ' adVarWChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param2", 203, 1, 1073741823, Request.Form("detail")) ' adLongVarWChar
    ...
    MM_editRedirectUrl = "feedback.asp?success=success"
    If (Request.QueryString <> "") Then
				  ...
				  <div class="row padded-h">
					  <div class="small-12 columns">
						...
						If (Request.QueryString("success") <> "") Then 
						  Response.Write "<script type='text/javascript'>alert('Gửi feedback thành công'); window.location='index.asp'</script>"
						End If
						%>
                                                ...
                                                <fieldset id="kaka">
                                                    ...
                                                    <div class="row">
                                                      <div class="large-12 columns">
                                                        <textarea name="detail" style="min-height: 7.5rem;" placeholder="Nhập nội dung" ></textarea>
                                                      </div>
                                                    </div>
                                                    <div class="form-actions">
                                                        <input type="submit" class="button" value="Gửi Feedback" />&nbsp;&nbsp;
                                                    </div>
                                                </fieldset>
						...
						<script type="text/javascript">
                        	function kt(){
								...
								if(title.length == 0){
									alert('Tiêu đề không thể bỏ trống');
									return false;
								}
								if(title.length > 100){
								...
								if(detail.length == 0){
									alert('Nội dung không thể bỏ trống');
									return false;
								}
								if(title.length <= 3){
									alert('Tiêu đề phải lớn hơn 3 kí tự');
									return false;	
								}
								if(detail.length <= 10){
									alert('Nội dung phải lớn hơn 10 kí tự');
									return false;	
								}
								return true;
							}
